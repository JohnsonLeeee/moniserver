...
class Context(object):
    ...
    def __init__(self,config = None):
        ...
        self.cur_no = 0
        self.your_no = 0
        self.lowbidprice = 82600
        
        self.timedelta = dt.datetime.now() - today_time(11, 29, 0)
    ...
    def process(self,clientdata):
        ...
        if "policy-file-request" in clientdata:
            self.msg_queue.put('<cross-domain-policy><allow-access-from domain="*" to-ports="*" /></cross-domain-policy>')
...
#             print msg
    ...
    def replys(self):
        ...
        if mock_time > today_time(11, 30, 8):
            self.timedelta = dt.datetime.now() - today_time(11, 29, 10)
            self.lowbidprice = 82600
...
#         print 'cur_no:%d, bid_no:%d, your_no:%d'%(self.cur_no,self.bid_no,self.your_no)
        ...
        if self.state == self.QUEUE_FINISH:
            r = self.nb.s2c0203("0", self.bidamount, self.bidnumber, (dt.datetime.now() - self.timedelta).strftime("%Y-%m-%d %H:%M:%S"), "出价有效", "2", dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), self.requestid)
            self.msg_queue.put(r)
            self.state = self.NORMAL
...
#         print 'reply 0-0'
    ...
    def reply2_2(self,msg):
        ...
        return self.nb.s2c0202("0","成功",bidamount,bidnumber,"出价入列，您处于第%d位，%d，%d"%(self.your_no,self.your_no,self.bid_no),"0",requestid,"0001-01-01 00:00:00")
